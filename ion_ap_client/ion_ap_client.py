...
class IonAPClient:
    """
    The main ion-AP API client interface
...
class PrintingIonAPClient(IonAPClient):
    """
    This class calls the same functions as the API client,
    but prints the output in a pretty format
    """
    def send_status_list(self, offset, limit):
        result, total = super().send_status_list(offset, limit)
        if result:

            print("Showing %d-%d (of %d transactions)" % (offset, min(total - 1, offset + limit - 1), total))
            rows = []
    ...
    def send_status_single(self, transaction_id):
        result = super().send_status_single(transaction_id)
        if result:
            print_dict_as_table(result)
    
    def send_status_errors(self, transaction_id):
        result = super().send_status_errors(transaction_id)
        if result:
    ...
    def send_status_logs(self, transaction_id):
        result = super().send_status_logs(transaction_id)
        if result:
            elements = result['results']
            rows = []
            for element in elements:
    ...
    def receive_list(self, offset, limit):
        ...
        if result:
            ...
            print("Showing %d-%d (of %d transactions)" % (offset, max(0, min(total-1, offset + limit - 1)), total))
            rows = []
            for element in elements:
                rows.append([
                    element['id'],
    ...
    def receive_single(self, transaction_id):
        result = super().receive_single(transaction_id)
        if result:
            print_dict_as_table(result)
    ...
    def receive_logs(self, transaction_id):
        result = super().receive_logs(transaction_id)
        if result:
            elements = result['results']
            rows = []
            for element in elements:
...
Main commands:
    send            Send document (derive SBDH if not present), then wait for initial result
    send_quick      Send document (derive SBDH if not present) without waiting
...
Use ion_ap_client <main command> -h for more details about the specific command.
    """)
        ...
        parser.add_argument("-w", "--wait", type=int, default=2, help="When sending, wait X seconds then check status (default: 2)")
...
""")
        ...
        if result:
            ...
            if self.send_wait_time > 0:
                print(f"Waiting {self.send_wait_time} seconds before requesting status")
